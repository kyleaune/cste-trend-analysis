...
mort.dat <- mort.dat %>%
  ...
  # 2010 to April 31, 2015, how would you adjust the following line? Do we need this line?
  filter(isoweek != "2017-W53-1" & isoweek != "2020-W53-1")

# To check if we need to filter the data, we can use any of the following approaches
min(mort.dat$isoweek)
max(mort.dat$isoweek)
...
pandemic.dates <- seq.Date(as.Date("2020-03-10"), # Question: What is the start date?
                           as.Date("2020-12-31"), # Question: What is the end date?
                           by = "day")
...
all.counts <- compute_counts(dat = mort.dat, # df with individual records
                             demo = demo.total, # df with population data
...
ggplot(weekly.all.plotdat, aes(x = date, y = value, col = class)) +
  ...
  scale_x_date(
    date_breaks = "1 month", # Question: What are the major and minor breaks?
...
# Question: What is the title of the graph?
  labs(title = "Observed and Expected Deaths in 2020", col = NULL)
...
# Answer: The expected deaths are lower than they were when calculated using
# longer timespan of data. Using less data under-estimates expected death in this case
...
mort.dat <- mort.dat %>%
  ...
  filter(isoweek != "2009-W53-1" & isoweek != "2020-W19-1")
...
flu.dates <- seq.Date(as.Date("2014-10-01"), # Question: What is the start date?
                           as.Date("2015-04-30"), # Question: What is the end date?
                           by = "day")
...
weekly.all.plotdat <- weekly.xs.all %>%
  ...
  # Question: Which dates will we include in the plot?
  filter(date > make_date(2014, 10, 1)) %>%
...
# Question: What is the title of the graph?
  labs(title = "Observed and Expected Deaths during Flu Season 2014-15 in Michigan", 
     col = NULL)
